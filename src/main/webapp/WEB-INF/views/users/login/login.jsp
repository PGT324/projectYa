            ...
            <div class="main">
                <div class="main-input1">
                    <form action="form_ok.php" method ="POST" class="inputform">
                        <input class="input1" name = "id" onkeyup="dataForm(this.form,event)" type="text" placeholder="이메일"/>
                        <div id = "t" style="color: red; font-size: 14px; margin-left: 30%; visibility: hidden;">아아</div>
				...
				success: function sc(s) {
					...
					if(s.USERS_IDX != null){
						...
						document.location.href = '${pageContext.request.contextPath}/users/loginForm/'+s.USERS_IDX;						
							...
							t.innerHTML = "이회원정보로는 로그인할수 없습니다";
						t.style.visibility = '';
						}
					}else{
						$.ajax({
							...
							type:"post",
							data:{id:email,pw:pw},
							success: function scs(s) {
								if(s.ADMIN_IDX != null){
									alert('로그인이 완료되었습니다')
									document.location.href = '${pageContext.request.contextPath}/admin/loginForm/'+s.ADMIN_IDX;
								}else{
									t.innerHTML = "입력한 회원정보를 다시 확인해주세요";
									t.style.visibility = '';									
								}
							}
						})
